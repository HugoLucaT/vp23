...
http.createServer(function(req, res){
	...
	if(req.method === 'POST'){
		...
		collectRequestData(req, result => {
			...
			fs.open('public/namelog.txt', 'a', (err, file)=>{
				if (err){
					...
					fs.appendFile('public/namelog.txt', '; ' + result.firstNameInput + ', ' + result.lastNameInput + ', ' + dateTimeValue.dateENformated(), (err)=>{
						if (err){
							throw err;
						}
						else{
		...
		res.write('\n\t<hr>\n\t<p><a href="addednames">Juba lisatud nimed!</a>!</p>');
	...
	else if (currentURL.pathname === "/addednames"){
		res.writeHead(200, {"Content-type": "text/html"});
		res.write(pageHead);
		res.write(pageBanner);
		res.write(pageBody);
